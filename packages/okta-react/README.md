...
Assuming you have configured your application to allow the `Authorization code` grant type, simply pass `grantType="authorization_code"` to the `Security` component. This will configure the auth object to perform PKCE flow for both login and token refresh.
